	...
	for(int c = 0; !tile_map_layout.eof(); c++)
	{
	    ...
	    std::string tile_coordinates_string = file_string.substr(0, 3);
            ...
            if(tile_coordinates_string.find(" ") == std::string::npos && tile_coordinates_string.find("\n") == std::string::npos)
            {
                ...
                m_tile_array[c % tiles_per_region.x][c / tiles_per_region.y] = tile_coordinates;
            }
	}
    }

    for(int c = 0; c < tiles_per_region.x * tiles_per_region.y; c++)
    {
        tiles.setPosition( ( c % tiles_per_region.x ) * pixels_per_tile.x, ( c / tiles_per_region.y ) * pixels_per_tile.y );
        tiles.setTextureRect(sf::IntRect( ( m_tile_array[c % tiles_per_region.x][c / tiles_per_region.y] % tiles_in_tile_atlas.x ) * pixels_per_tile.x,
                                          ( m_tile_array[c % tiles_per_region.x][c / tiles_per_region.y] / tiles_in_tile_atlas.y ) * pixels_per_tile.y,
                                            pixels_per_tile.x, pixels_per_tile.y));

        main_window.draw(tiles);
    }
}
